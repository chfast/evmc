...
//export setStorage
func setStorage(pCtx unsafe.Pointer, pAddr *C.struct_evmc_address, pKey *C.struct_evmc_uint256be, pVal *C.struct_evmc_uint256be) {
	idx := int((*C.struct_extended_context)(pCtx).index)
	ctx := getHostContext(idx)
	ctx.SetStorage(goAddress(*pAddr), goHash(*pKey), goHash(*pVal))
}
